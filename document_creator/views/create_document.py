from flask import request, Blueprint, Response, jsonify, current_app, make_response, render_template
...
from document_creator.views.upload_document import document_upload
import os
from document_creator.views import create_pdf
...
import pdfkit
import requests
from document_creator.views import  property_info
import pdfrw
import base64
from PIL import Image
import io
# This is the blueprint object that gets registered into the app in blueprints.py.
create_documents = Blueprint('create_documents', __name__,
                    template_folder='templates')


@create_documents.route("/create_pdf_report_external", methods=['POST'])
def create_pdf_report_external():
    ...
    data= property_info.get_info(json_data)
    ...
    if headingvaluefound == "true":
        ...
        urlretrieve(url1, f"/opt/document_creator/views/templates/pdf_templates/streetmapimage1.jpeg")
        urlretrieve(url2, f"/opt/document_creator/views/templates/pdf_templates/streetmapimage2.jpeg")
        urlretrieve(url3, f"/opt/document_creator/views/templates/pdf_templates/streetmapimage3.jpeg")

        urlretrieve(urlpov, f"/opt/document_creator/views/templates/pdf_templates/streeimagepov.jpeg")
    else:
        url=None
        url1=None
        url2=None
    ...
    if get_polygon_info != None:
        urlretrieve(get_polygon_info, f"/opt/document_creator/views/templates/pdf_templates/polygonimage.jpeg")
    ...
    if data.get('cafes',{}).get('results',[]) != []:
        ...
        for cafe in data.get('cafes',{}).get('results',[]):
            locationscafe.append({"lat":cafe['geometry']['location']['lat'],"lng":cafe['geometry']['location']['lng'],'label': str(cafeno)})
    ...
    if locationscafe != []:
        ...
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationscafe])
        ...
        with open('/opt/document_creator/views/templates/pdf_templates/mapcafe.png', 'wb') as f:
    ...
    if data.get('supermarkets',{}).get('results',[]) != []:
        ...
        for cafe in data.get('supermarkets',{}).get('results',[]):
            locationssupermarkets.append({"lat":cafe['geometry']['location']['lat'],"lng":cafe['geometry']['location']['lng'],'label': str(supermarketsno)})
    ...
    if locationssupermarkets != []:
        ...
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationssupermarkets])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'

        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mapsmarkets.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('restaraunts',{}).get('results',[]) != []:
        ...
        for gym in data.get('restaraunts',{}).get('results',[]):
            locationrestaraunts.append({"lat":gym['geometry']['location']['lat'],"lng":gym['geometry']['location']['lng'],'label': str(restarauntsno)})
    ...
    if locationrestaraunts != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationrestaraunts])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/maprestaraunts.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('lon_lat',{}) != {}:
        ...
        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/maplocation.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('nearby_schools',{}).get('independent',{}) != {}:
        for gym in data.get('nearby_schools',{}).get('independent',{}).get('Boarding',[]):
            locationsschoolsindependant.append({"lat":gym['lat'],"lng":gym['lng'],'label': str(schoolsnoind)})
    ...
    if data.get('nearby_schools',{}).get('state',{}) != {}:
        for gym in data.get('nearby_schools',{}).get('state',{}).get('Primary',[]):
            locationsschoolstate.append({"lat":gym['lat'],"lng":gym['lng'],'label': str(schoolsnostate)})
    ...
    if locationsschoolsindependant != []:
        ...
        min_lat = min(location['lat'] for location in locationsschoolsindependant)
        max_lat = max(location['lat'] for location in locationsschoolsindependant)
        min_lng = min(location['lng'] for location in locationsschoolsindependant)
        max_lng = max(location['lng'] for location in locationsschoolsindependant)
        path = f'{min_lat},{min_lng}|{min_lat},{max_lng}|{max_lat},{max_lng}|{max_lat},{min_lng}|{min_lat},{min_lng}'


        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationsschoolsindependant])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'

        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mapschoolsindependant.png', 'wb') as f:
            f.write(response.content)
    ...
    if locationsschoolstate != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationsschoolstate])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mapschoolstate.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('gyms',{}).get('results',[]) != []:
        ...
        for gym in data.get('gyms',{}).get('results',[]):
            locationsgyms.append({"lat":gym['geometry']['location']['lat'],"lng":gym['geometry']['location']['lng'],'label': str(gymsno)})
    ...
    if locationsgyms != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationsgyms])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mapgym.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('post_offcies',{}).get('results',[]) != []:
        ...
        for gym in data.get('post_offcies',{}).get('results',[]):
            locationspost_office.append({"lat":gym['geometry']['location']['lat'],"lng":gym['geometry']['location']['lng'],'label': str(post_officeo)})
    ...
    if locationspost_office != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in locationspost_office])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mappostoffice.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('get_bus_and_tubes',[])!= []:
        ...
        for gym in data.get('get_bus_and_tubes',[]):
            if gym['type'] == "bus_stop":
                buslist.append({"lat":gym['latitude'],"lng":gym['longitude'],'label': str(busno)})
    ...
    if buslist != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in buslist])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/mapbuses.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('get_train_stations',[]) != []:
        ...
        for gym in data.get('get_train_stations',[]):
            trainlist.append({"lat":gym['latitude'],"lng":gym['longitude'],'label': str(trainno)})
    ...
    if trainlist != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in trainlist])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/maptrain.png', 'wb') as f:
            f.write(response.content)
    ...
    if data.get('get_bus_and_tubes',[])!= []:
        ...
        for gym in data.get('get_bus_and_tubes',[]):
            if gym['type'] == "tube_station":
                tubelist.append({"lat":gym['latitude'],"lng":gym['longitude'],'label': str(tubeno)})
    ...
    if tubelist != []:
        ...
        # Construct the URL for the API request with markers
        markers = '&'.join([f'markers=color:red%7Clabel:{location["label"]}%7C{location["lat"]},{location["lng"]}' for location in tubelist])
        url = f'https://maps.googleapis.com/maps/api/staticmap?size={size}&path=color:0x0000ff|weight:1|fillcolor:0xFFFF0033|{path}&key={api_key}&{markers}&{home_mark}'


        # Make the GET request and save the image
        response = requests.get(url)
        with open('/opt/document_creator/views/templates/pdf_templates/maptube.png', 'wb') as f:
            f.write(response.content)
    ...
    print(address)
    new_doc_name= "{}_{}".format(pdf_template,address['address'])
    data.update(address)
    # shutil.copy(f"{config.doc_location}pdf_templates/{template_name}.pdf", f"{config.doc_location}pdf_templates_copy/")
    create_pdf_report(f"pdf_templates/{pdf_template}.html", data, f"/opt/document_creator/views/templates/pdf_templates/{new_doc_name}.pdf", json_data['logo_url'],headingvaluefound, get_polygon_info, tubes)

    upload=document_upload('property_reports', f"/opt/document_creator/views/templates/pdf_templates/{new_doc_name}.pdf", new_doc_name, "pdf")

    os.remove(f"/opt/document_creator/views/templates/pdf_templates/{new_doc_name}.pdf")
    if headingvaluefound == "true":

        os.remove(f"/opt/document_creator/views/templates/pdf_templates/streetmapimage.jpeg")
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/streetmapimage1.jpeg")
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/streetmapimage2.jpeg")
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/streetmapimage3.jpeg")
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/streeimagepov.jpeg")
    if get_polygon_info != None:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/polygonimage.jpeg")
    if locationscafe != []:

        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapcafe.png")
    if locationssupermarkets != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapsmarkets.png")
    if locationsgyms != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapgym.png")

    if locationspost_office != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mappostoffice.png")
    if locationrestaraunts != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/maprestaraunts.png")
    if tubelist != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/maptube.png")
    if trainlist != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/maptrain.png")
    if buslist != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapbuses.png")
    if locationsschoolstate != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapschoolstate.png")
    
    if locationsschoolsindependant != []:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/mapschoolsindependant.png")

    if data.get('lon_lat',{}) != {}:
        os.remove(f"/opt/document_creator/views/templates/pdf_templates/maplocation.png")

    return jsonify({'document_name': new_doc_name})

def create_pdf_report(input_pdf_path, data, output_pdf_path, logo, headerfound, polygonimagemap, tubes):
    if data.get('lon_lat') != {}:
        if headerfound == "true":
            street_image_url=f"/opt/document_creator/views/templates/pdf_templates/streetmapimage.jpeg"
            street_image_url2=f"/opt/document_creator/views/templates/pdf_templates/streetmapimage1.jpeg"
            street_image_url3=f"/opt/document_creator/views/templates/pdf_templates/streetmapimage2.jpeg"
            street_image_url4=f"/opt/document_creator/views/templates/pdf_templates/streetmapimage3.jpeg"
            streeimagepov=f"/opt/document_creator/views/templates/pdf_templates/streeimagepov.jpeg"
    ...
    if polygonimagemap != None:
        polygonimage=f"/opt/document_creator/views/templates/pdf_templates/polygonimage.jpeg"
    else:
        polygonimage=None
    mapcafe=f"/opt/document_creator/views/templates/pdf_templates/mapcafe.png"
    mapsmarkets=f"/opt/document_creator/views/templates/pdf_templates/mapsmarkets.png"
    mapgym=f"/opt/document_creator/views/templates/pdf_templates/mapgym.png"
    mappostoffice=f"/opt/document_creator/views/templates/pdf_templates/mappostoffice.png"
    maprestaraunts=f"/opt/document_creator/views/templates/pdf_templates/maprestaraunts.png"
    maptrain=f"/opt/document_creator/views/templates/pdf_templates/maptrain.png"
    mapschoolstate=f"/opt/document_creator/views/templates/pdf_templates/mapschoolstate.png"
    mapschoolsindependant=f"/opt/document_creator/views/templates/pdf_templates/mapschoolsindependant.png"

    maptube=None
    if tubes != False:
        maptube=f"/opt/document_creator/views/templates/pdf_templates/maptube.png"
    mapbuses=f"/opt/document_creator/views/templates/pdf_templates/mapbuses.png"
    maplocation=f"/opt/document_creator/views/templates/pdf_templates/maplocation.png"


    template= render_template(f'{input_pdf_path}',
                            data=data,
                            tubes=tubes,
                            logo=logo,
    ...
    pdf = pdfkit.from_string(template, output_pdf_path)
...
def get_image_file_as_base64_data(filepath):
    with open(f"{config.doc_location}/templates/pdf_templates/{filepath}", 'rb') as image_file:
